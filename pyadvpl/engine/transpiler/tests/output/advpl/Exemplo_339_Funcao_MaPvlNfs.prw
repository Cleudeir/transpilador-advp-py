...
USER FUNCTION zExe339()
    LOCAL aArea, cPedido, aPvlDocS, cDocument, cSerie, cFunBkp, lAbortar
    ...
    If .NOT. lAbortar
        ...
        If SC5:MsSeek(FWxFilial("SC5") + cPedido) .AND. Empty(SC5->C5_NOTA)
            ...
            While .NOT. SC6->( DbEof() ) .AND. SC6->C6_FILIAL = SC5->C5_FILIAL
                ...
                If Empty(SC9->C9_BLEST) .AND. Empty(SC9->C9_BLCRED)
                    aAdd(aPvlDocS, { SC9->C9_PEDIDO, SC9->C9_ITEM, SC9->C9_SEQUEN, SC9->C9_QTDLIB, SC9->C9_PRCVEN, SC9->C9_PRODUTO, .F., SC9:RecNo(), SC5:RecNo(), SC6:RecNo(), SE4:RecNo(), SB1:RecNo(), SB2:RecNo(), SF4:RecNo() })
                EndIf
                SC6:DbSkip()
            EndDo
            If Len(aPvlDocS) > 0
                cFunBkp := FunName()
                SetFunName("MATA461")
                cDocument := MaPvlNfs(aPvlDocS, cSerie, .F., .F., .F., .T., .F., 0, 0, .T., .F., "", Nil, Nil, Nil, Nil, Date(), .F.)
                SetFunName(cFunBkp)
                If Empty(cDocument)
                    lAbortar := .T.
                Else
