...
USER FUNCTION zExe146()
    LOCAL aArea, aCabec, aDados, aCampos, aHeader, aCols, cQryAux
    ...
    aAdd(aHeader, { "Código", "B1_COD", "", TamSX3("B1_COD")[1], 0, ".T.", ".T.", "C", "", "" })
    aAdd(aHeader, { "Descrição", "B1_DESC", "", TamSX3("B1_DESC")[1], 0, ".T.", ".T.", "C", "", "" })
    aAdd(aHeader, { "Tipo", "B1_TIPO", "", TamSX3("B1_TIPO")[1], 0, ".T.", ".T.", "C", "", "" })
    aAdd(aHeader, { "Unidade Medida", "B1_UM", "", TamSX3("B1_UM")[1], 0, ".T.", ".T.", "C", "", "" })
    // Busca os produtos
    ...
    While .NOT. QRY_AUX->( DbEof() )
        aAdd(aCols, { QRY_AUX->B1_COD, QRY_AUX->B1_DESC, QRY_AUX->B1_TIPO, QRY_AUX->B1_UM, .F. })
        QRY_AUX:DbSkip()
    EndDo
    QRY_AUX:DbCloseArea()
