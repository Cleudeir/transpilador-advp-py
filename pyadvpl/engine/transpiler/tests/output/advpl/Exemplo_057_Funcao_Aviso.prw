...
USER FUNCTION zExe057()
    ...
    Aviso("Título Exemplo 3 (Editável)", @ cMsgRet, { "OK" }, 3, "Sub Título", Nil, Nil, .T.)
    FWAlertInfo(cMsgRet, "Conteúdo digitado")
    // Mensagem que fecha sozinha depois de 5 segundos
    cMsg += " (tela será fechada em 5 segundos)"
    Aviso("Título Exemplo 4 (Timer)", cMsg, { "OK" }, 2, "Sub Título", Nil, Nil, Nil, 5000)
    FWRestArea(aArea)
    RETURN
