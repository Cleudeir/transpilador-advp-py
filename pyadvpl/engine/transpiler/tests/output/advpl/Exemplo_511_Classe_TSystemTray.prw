...
FUNCTION fMontaTela()
    LOCAL nLinObj, nLargBtn, nAltuBtn, bConfirm, bCancela, cFontPad, oFontBtn, oFontBtnN, oFontMod, oFontMaior, oFontSub, lCentered, oBtConfirm, oBtCancela, oSayTitulo, cSayTitulo, oSayEtiqus, cSayEtiqus, oDlgCentral, aTamanho, nJanLarg, nJanAltu, nPosTop, nPosLeft, cEspacProd, oGetProdut, cGetProdut, oGetBlank, cGetBlank, oGridPro, aHeaderPro, aColsPro, oMenu, oItem1, oItem2, oItem3, cIcon, oSysTray
    ...
    If lProgInic
        ...
        oMenu := TMenu():New(0, 0, 0, 0, .T., Nil, oDlgCentral)
        oItem1 := TMenuItem():New(oDlgCentral, "Mudar a Cor de Fundo", Nil, Nil, Nil, Nil, Nil, "BR_VERDE", Nil, Nil, Nil, Nil, Nil, Nil, .T.)
        oItem2 := TMenuItem():New(oDlgCentral, "Exibir Data e Hora", Nil, Nil, Nil, Nil, Nil, "HISTORIC", Nil, Nil, Nil, Nil, Nil, Nil, .T.)
        oItem3 := TMenuItem():New(oDlgCentral, "Encerrar Aplicação", Nil, Nil, Nil, Nil, Nil, "EXCLUIR", Nil, Nil, Nil, Nil, Nil, Nil, .T.)
        oMenu:Add(oItem1)
        oMenu:Add(oItem2)
        oMenu:Add(oItem3)
