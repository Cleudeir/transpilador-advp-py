...
USER FUNCTION zExe474()
    ...
    DbUseArea(.T., "TOPCONN", TCGenQry(Nil, Nil, cQrySBM), "QRY_TST", .F., .T.)
    // Caso haja dados, exibe uma mensagem
    If .NOT. QRY_TST->( DbEof() )
    ...
    aAdd(aParSubst, "G001")
    // Abre um novo alias conforme a query passada
    DbUseArea(.T., "TOPCONN", TCGenQry2(Nil, Nil, cQrySBM, aParSubst), "QRY_TST", .F., .T.)
    // Caso haja dados, exibe uma mensagem
    If .NOT. QRY_TST->( DbEof() )
