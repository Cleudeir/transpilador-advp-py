...
USER FUNCTION zExe338()
    ...
    If SC5:MsSeek(FWxFilial("SC5") + cPedido) .AND. Empty(SC5->C5_NOTA)
        ...
        While .NOT. SC6->( DbEof() ) .AND. SC6->C6_FILIAL = SC5->C5_FILIAL
            ...
            While .NOT. SC9->( DbEof() ) .AND. SC9->C9_FILIAL + C9_PEDIDO + C9_ITEM = FWxFilial("SC9") + SC6->C6_NUM + C6_ITEM
                SC9:a460Estorna(.T.)
                SC9:DbSkip()
            EndDo
            SC6:DbSkip()
        ...
        SC6:DbGoTop()
        SC6:MsSeek(SC5->C5_FILIAL + SC5->C5_NUM)
        While .NOT. SC6->( DbEof() ) .AND. SC5->C5_NUM = SC6->C6_NUM
            MaLibDoFat(SC6:RecNo(), SC6->C6_QTDVEN, Nil, Nil, .T., .T., .F., .F.)
            SC6:DbSkip()
        EndDo
    EndIf
    FWRestArea(aArea)
