...
USER FUNCTION zExe297()
    LOCAL aArea, cMensagem, cSolucao

    aArea := FWGetArea()
    cMensagem := "Não foi encontrado parametrização para executar a rotina"
    cSolucao := "Cadastre o parâmetro MV_X_PARAM"
    // Exibindo a mensagem com Help
    Help(Nil, Nil, "Help", Nil, cMensagem, 1, 0, Nil, Nil, Nil, Nil, Nil, { cSolucao })
    // Exibindo a mensagem com ExibeHelp
    ExibeHelp("Help", cMensagem, cSolucao)
    // Exibindo a mensagem com ShowHelpDlg
    ShowHelpDlg("Help", { cMensagem }, Nil, { cSolucao })
    FWRestArea(aArea)
    RETURN
