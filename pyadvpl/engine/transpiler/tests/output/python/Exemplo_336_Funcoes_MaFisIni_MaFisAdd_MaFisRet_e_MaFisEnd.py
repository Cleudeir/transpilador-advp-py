...
def u_zExe336():
    ...
    if SC5.MsSeek(FWxFilial('SC5') + cPedido):
        MaFisIni(SC5.C5_CLIENTE, SC5.C5_LOJACLI, ('F' if SC5.C5_TIPO in 'D;B' else 'C'), SC5.C5_TIPO, SC5.C5_TIPOCLI, MaFisRelImp('MT100', ['SF2', 'SD2']), None, None, 'SB1', 'MATA461')
        # Enquanto houver itens
        SC6.MsSeek(FWxFilial('SC6') + SC5.C5_NUM)
        aAreaBkp = SC6.FWGetArea()
        nItAtu = 0
        while not SC6.EoF() and SC6.C6_NUM == SC5.C5_NUM:
            nItAtu += 1
        ...
        if SC5.C5_DESCONT > 0:
            MaFisAlt('NF_DESCONTO', Min(MaFisRet(None, 'NF_VALMERC') - 0.01, SC5.C5_DESCONT + MaFisRet(None, 'NF_DESCONTO')))

        if SC5.C5_PDESCAB > 0:
            MaFisAlt('NF_DESCONTO', A410Arred(MaFisRet(None, 'NF_VALMERC') * SC5.C5_PDESCAB / 100, 'C6_VALOR') + MaFisRet(None, 'NF_DESCONTO'))
        ...
        nTotFrete = MaFisRet(None, 'NF_FRETE')
        nTotVal = MaFisRet(None, 'NF_TOTAL')
        MaFisEnd()
