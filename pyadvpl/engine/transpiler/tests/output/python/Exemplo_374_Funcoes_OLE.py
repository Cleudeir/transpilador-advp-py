...
def fMontaRel():
    nHandWord = 0
    cPastaTmp = GetTempPath()
    cArqOrigi = '\\x_dots\\produto.dotx'
    cArqLocal = cPastaTmp + 'produtos.dotx'
    cArqPDF = cPastaTmp + 'produtos.pdf'
    DbSelectArea('SB1')
    SB1.DbSetOrder(1)
    # Filial + Produto
    if SB1.MsSeek(FWxFilial('SB1') + MV_PAR01):
        ...
        OLE_SetProperty(nHandWord, '208', False)
        OLE_SaveAsFile(nHandWord, cArqPDF, None, None, False, 17)
