        ...
        if SC5.MsSeek(FWxFilial('SC5') + cPedido) and Empty(SC5.C5_NOTA):
            ...
            while not SC6.Eof() and SC6.C6_FILIAL == SC5.C5_FILIAL and SC6.C6_NUM == SC5.C5_NUM:
                ...
                if Empty(SC9.C9_BLEST) and Empty(SC9.C9_BLCRED):
                    aAdd(aPvlDocS, [SC9.C9_PEDIDO, SC9.C9_ITEM, SC9.C9_SEQUEN, SC9.C9_QTDLIB, SC9.C9_PRCVEN, SC9.C9_PRODUTO, False, SC9.RecNo(), SC5.RecNo(), SC6.RecNo(), SE4.RecNo(), SB1.RecNo(), SB2.RecNo(), SF4.RecNo()])
            ...
            if Len(aPvlDocS) > 0:
                cFunBkp = FunName()
                SetFunName('MATA461')
                cDocument = MaPvlNfs(aPvlDocS, cSerie, False, False, False, True, False, 0, 0, True, False, '', lambda : None, lambda : None, lambda : None, lambda : True, Date(), False)
                SetFunName(cFunBkp)
                if Empty(cDocument):
                    lAbortar = True
                else:
